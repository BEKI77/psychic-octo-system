...
function MobileBottomNav() {
  const pathname = usePathname();
  const { data: me } = useMe();
  const items = NAV_GROUPS.flatMap((group) => group.items).filter((item) => !item.requires || me?.permissions.includes(item.requires)).slice(0, 5);
  return <nav aria-label="Primary navigation" className="fixed inset-x-3 bottom-3 z-30 flex h-16 items-center justify-around rounded-2xl border border-border/80 bg-card/95 px-1 shadow-2xl shadow-primary/10 backdrop-blur-xl md:hidden">
    {items.map((item) => {
      const Icon = item.icon;
      const active = pathname === item.href;
      return <Link key={item.href} href={item.href} className={cn("flex min-w-14 flex-col items-center gap-1 rounded-xl px-2 py-2 text-[10px] font-medium text-muted-foreground transition-colors", active && "bg-primary/10 text-primary")}>
        <Icon className="size-5" /><span>{item.label}</span>
      </Link>;
    })}
  </nav>;
}
